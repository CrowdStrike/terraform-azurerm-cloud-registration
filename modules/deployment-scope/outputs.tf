output "active_subscriptions_by_group" {
  ...
  value = {
    ...
    mg => [
      for sub in data.azurerm_subscription.subscriptions_mg : sub.subscription_id if sub.state == "Enabled" && contains(subs, sub.subscription_id)
    ]
  }
}
...
output "all_active_subscription_ids" {
  ...
  value = toset(
    flatten(concat(var.subscription_ids, [
      for sub in data.azurerm_subscription.subscriptions_mg : sub.subscription_id if sub.state == "Enabled"
    ]))
  )
}
