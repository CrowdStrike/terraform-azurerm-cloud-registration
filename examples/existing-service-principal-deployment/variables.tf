# Service Principal Information (provided by Identity Team)
variable "crowdstrike_service_principal_object_id" {
  description = "Object ID of the CrowdStrike service principal created by the Identity Team. This is required to assign Azure role permissions."
  type        = string

  validation {
    condition     = can(regex("^[0-9a-f]{8}-([0-9a-f]{4}-){3}[0-9a-f]{12}$", var.crowdstrike_service_principal_object_id))
...
variable "falcon_client_id" {
  type        = string
  sensitive   = true
  description = "Falcon API client ID. Required for CrowdStrike tenant registration and API operations."
...
variable "falcon_client_secret" {
  type        = string
  sensitive   = true
  description = "Falcon API client secret. Required for CrowdStrike tenant registration and API operations."
